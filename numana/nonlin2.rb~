...
<JHLP lab="poq">
<LI><CODE>f</CODE> does a single step of long division. To
understand this consider dividing <M>2+3x+4x^2-5x^3</M>
by <M>1+2x^2.</M> In a single step you'll look at the highest
terms, and divide one by the other to get <M>[[52]]x</M> and
subtract this times the divisor from the dividend. J will treat
the divisor as <CODE>1 0 2</CODE> and dividend as <CODE>2 3 4
-5</CODE>. The single step is implemented as 
<CODE>1 0 2 f 2 3 4 -5</CODE>. The code first takes the highest
coefficients (the tails): <CODE>{:y</CODE> (e.g., 5)
and <CODE>{:x</CODE> (e.g., 2), divides
one by the other to get <CODE>({:y) % {:x</CODE> (e.g., <M>[[52]]</M>), and then
multiplies it with the divisor <CODE>x</CODE> (e.g. <M>[[52]](1+2x^2)</M>), and adds the
appropriate number (degree difference) of zeroes to achieve the
correct power (e.g., <M>[[52]]x(1+2x^2).</M> Finally it subtracts
this from the dividend <CODE>y</CODE>.
</LI>
...
Let <M>a<b\in\rr</M> be two numbers that are not zeros of any of the
<M>p_i</M>'s. Then <M>\sigma(a)-\sigma(b)</M> is the number of
